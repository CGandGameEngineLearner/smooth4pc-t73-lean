...
class P0JohnsonCertificateTest(unittest.TestCase):
    def test_fast_certificate_has_only_geometric_braid_pending(self):
        ...
        self.assertTrue(result["checks"]["johnson_ar_affine_bridge"])
        self.assertFalse(result["checks"]["actual_heegaard_preserving_psi_A"])
        self.assertFalse(result["checks"]["actual_framed_ar_link"])
        self.assertFalse(result["checks"]["two_cancellations"])
        self.assertFalse(result["checks"]["actual_cut_tangle"])
