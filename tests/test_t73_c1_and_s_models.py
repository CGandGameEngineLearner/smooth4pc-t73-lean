...
class SStandardSpheresTest(unittest.TestCase):
    def test_movies_miss_the_p0_ball_but_s_stays_open(self) -> None:
        result = load("certify_t73_s_standard_spheres").generate()
        ...
        self.assertFalse(result["checks"]["detector_fixed"])
        self.assertFalse(result["checks"]["actual_standard_sphere_endpoint_foam_computed"])
        ...
        self.assertFalse(result["checks"]["actual_attaching_system_identified"])
        ...
        self.assertFalse(result["attaching_homology"]["identified_with_actual_attaching_system"])
