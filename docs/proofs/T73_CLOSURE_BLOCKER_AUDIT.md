...
**Status:** `OPEN` (P0, C and S discharged; P3 remains Open)
...
- No full-formal-q W2 functional is asserted.  The finite cubic `2624` is
  computed; P3 remains Open, so there is no candidate-level cocone.
...
C remains discharged only for the Johnson replacement collar; S is discharged
for the reversed 1-handle picture; see
