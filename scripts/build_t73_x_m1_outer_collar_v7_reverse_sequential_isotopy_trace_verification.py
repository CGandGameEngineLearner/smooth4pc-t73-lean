#!/usr/bin/env python3
"""Save/check independent verification of the reverse V7 schedule."""
...
BUILDER = (
    ROOT / "scripts/build_t73_x_m1_outer_collar_v7_reverse_sequential_isotopy_trace.py"
)
VERIFIER = (
    ROOT / "scripts/verify_t73_x_m1_outer_collar_v7_reverse_sequential_isotopy_trace.py"
)
...
def load_verifier():
    spec = importlib.util.spec_from_file_location("v7_reverse_trace_verifier", VERIFIER)
...
def check_result(result):
    ...
    if result != expected:
        raise AssertionError(f"V7 reverse trace result changed: {result}")
...
def build_full():
    ...
    check_result(result)
    receipt = {
        "schema": "t73_x_m1_outer_collar_v7_reverse_sequential_isotopy_trace_verification/v1",
...
def check_files(receipt):
    ...
    if (
        ...
        raise AssertionError("V7 reverse verification binding changed")
...
def main():
    ...
    check_files(receipt)
    print(f"T73_X_M1_V7_REVERSE_TRACE={receipt['verdict']}")
