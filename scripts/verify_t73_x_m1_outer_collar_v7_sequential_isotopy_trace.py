#!/usr/bin/env python3
"""Independently replay the rational sequential V7 collar schedule."""
...
def cylinder(path, start, end):
    return [(*value, start) for value in path] + [(*value, end) for value in path]


def rescale(values, start, end):
    width = end - start
    result = []
    for value in values:
        coordinates = point(value)
        result.append((*coordinates[:3], start + coordinates[3] * width))
    return result
...
def expected_family(vertices, cells):
    if not all(rank_two(vertices, cell) for cell in cells):
        raise AssertionError("independent sequential R4 rank check failed")
    return encode(vertices), cells
...
def verify_full():
    ...
    if (
        ...
        raise AssertionError("sequential trace is stale relative to verified V1")
    path = resolve(data["cache_path"])
    if (
        path.stat().st_size != data["cache_size"]
        or file_sha(path) != data["cache_sha256"]
    ):
        raise AssertionError("sequential trace cache bytes changed")
    ...
    with (
        ...
        if header[
            "schema"
        ] != "t73_x_m1_outer_collar_v7_sequential_isotopy_trace/v1" or not header[
            "classification"
        ].startswith("CANDIDATE_UNVERIFIED"):
            raise AssertionError("sequential trace header changed")
        for position, (old_line, new_line) in enumerate(
            ...
            if start != previous_end:
                raise AssertionError(
                    "sequential moving time slots overlap or leave a gap"
                )
            previous_end = end
            ...
            families = {
                ...
                "final_core": (
                    cylinder(final_core, middle, Fraction(1)),
                    cells,
                ),
                "source_push": (
            ...
            if any(new.get(key) != value for key, value in expected.items()):
                raise AssertionError("sequential trace reconstruction changed")
            counts["records"] += 1
            counts["boundary_matches"] += 5
    if previous_end != 1:
        raise AssertionError("sequential schedule does not cover global time")
    if digest.hexdigest().upper() != data["record_stream_sha256"]:
        raise AssertionError("sequential decompressed stream changed")
    ...
    if counts != expected_counts:
        raise AssertionError(f"sequential replay totals changed: {counts}")
    return {
