#!/usr/bin/env python3
"""Save/check independent verification of the sequential ribbon volume."""
...
BUILDER = (
    ROOT / "scripts/build_t73_x_m1_outer_collar_v7_sequential_framed_isotopy_volume.py"
)
VERIFIER = (
    ROOT / "scripts/verify_t73_x_m1_outer_collar_v7_sequential_framed_isotopy_volume.py"
)
...
def load_verifier():
    spec = importlib.util.spec_from_file_location("v7_framed_volume_verifier", VERIFIER)
...
def check_result(result):
    ...
    if result != expected:
        raise AssertionError(f"V7 framed-volume result changed: {result}")
...
def build_full():
    ...
    check_result(result)
    receipt = {
        "schema": "t73_x_m1_outer_collar_v7_sequential_framed_isotopy_volume_verification/v1",
...
def check_files(receipt):
    ...
    if (
        ...
        raise AssertionError("V7 framed-volume verification binding changed")
...
def main():
    ...
    check_files(receipt)
    print(f"T73_X_M1_V7_FRAMED_VOLUME={receipt['verdict']}")
