#!/usr/bin/env python3
"""Save/check independent local verification of outer collars v5."""
...
OUTPUT = ROOT / "audit/t73_x_m1_framed_outer_interface_collars_v5_verification.json"
BUILDER = ROOT / "scripts/build_t73_x_m1_framed_outer_interface_collars_v5.py"
VERIFIER = ROOT / "scripts/verify_t73_x_m1_framed_outer_interface_collars_v5.py"
...
def load_verifier():
    spec = importlib.util.spec_from_file_location("outer_collar_v5_verifier", VERIFIER)
...
def check_result(result):
    expected = {
        ...
        "changed_dual_collars": 8,
        "unchanged_johnson_collars": 3018,
    ...
    if result != expected:
        raise AssertionError(f"v5 outer collar result changed: {result}")
...
def build_full():
    ...
    check_result(result)
    receipt = {
        "schema": "t73_x_m1_framed_outer_interface_collars_v5_verification/v1",
...
def check_files(receipt):
    ...
    if (
        ...
        raise AssertionError("v5 outer collar verification binding changed")
...
def main():
    ...
    check_files(receipt)
    print(f"T73_X_M1_OUTER_COLLARS_V5={receipt['verdict']}")
