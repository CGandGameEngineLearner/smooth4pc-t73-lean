#!/usr/bin/env python3
"""Save/check independent local verification of outer collars v7."""
...
OUTPUT = ROOT / "audit/t73_x_m1_framed_outer_interface_collars_v7_verification.json"
BUILDER = ROOT / "scripts/build_t73_x_m1_framed_outer_interface_collars_v7.py"
VERIFIER = ROOT / "scripts/verify_t73_x_m1_framed_outer_interface_collars_v7.py"
...
def load_verifier():
    spec = importlib.util.spec_from_file_location("outer_collar_v7_verifier", VERIFIER)
...
def check_result(result):
    expected = {
        ...
        "historical_collision_exact_rechecks": 2,
    ...
    if result != expected:
        raise AssertionError(f"v7 outer collar result changed: {result}")
...
def build_full():
    ...
    check_result(result)
    receipt = {
        "schema": "t73_x_m1_framed_outer_interface_collars_v7_verification/v1",
...
def check_files(receipt):
    ...
    if (
        ...
        raise AssertionError("v7 outer collar verification binding changed")
...
def main():
    ...
    check_files(receipt)
    print(f"T73_X_M1_OUTER_COLLARS_V7={receipt['verdict']}")
