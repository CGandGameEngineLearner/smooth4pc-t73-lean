#!/usr/bin/env python3
"""Save/check independent local verification of outer collars v6."""
...
OUTPUT = ROOT / "audit/t73_x_m1_framed_outer_interface_collars_v6_verification.json"
BUILDER = ROOT / "scripts/build_t73_x_m1_framed_outer_interface_collars_v6.py"
VERIFIER = ROOT / "scripts/verify_t73_x_m1_framed_outer_interface_collars_v6.py"
...
def load_verifier():
    spec = importlib.util.spec_from_file_location("outer_collar_v6_verifier", VERIFIER)
...
def check_result(result):
    expected = {
        ...
        "changed_after_dual_collars": 4,
        "unchanged_collars": 3022,
        "normal_transversality_checks": 18156,
        "local_ribbon_star_checks": 15134,
        "local_ribbon_star_relation_counts": {
            "same_interface/TRANSVERSE_PLANES": 15130,
            "shared_dual_germ/COPLANAR_OPPOSITE_SIDES": 4,
        },
        "former_v5_collision_exact_rechecks": 1,
    ...
    if result != expected:
        raise AssertionError(f"v6 outer collar result changed: {result}")
...
def build_full():
    ...
    check_result(result)
    receipt = {
        "schema": "t73_x_m1_framed_outer_interface_collars_v6_verification/v1",
...
def check_files(receipt):
    ...
    if (
        ...
        raise AssertionError("v6 outer collar verification binding changed")
...
def main():
    ...
    check_files(receipt)
    print(f"T73_X_M1_OUTER_COLLARS_V6={receipt['verdict']}")
