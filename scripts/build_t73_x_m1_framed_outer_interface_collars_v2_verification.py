#!/usr/bin/env python3
"""Save/check independent local verification of outer collars v2."""
...
OUTPUT = ROOT / "audit/t73_x_m1_framed_outer_interface_collars_v2_verification.json"
BUILDER = ROOT / "scripts/build_t73_x_m1_framed_outer_interface_collars_v2.py"
VERIFIER = ROOT / "scripts/verify_t73_x_m1_framed_outer_interface_collars_v2.py"
...
def load_verifier():
    spec = importlib.util.spec_from_file_location("outer_collar_v2_verifier", VERIFIER)
...
def check_result(result):
    expected = {
        ...
        "core_push_segments_each": 6052,
        "ribbon_triangles_reconstructed": 12104,
        "phase_one_exact_parameter_checks": 9078,
    ...
    if result != expected:
        raise AssertionError(f"v2 outer collar result changed: {result}")
...
def build_full():
    ...
    check_result(result)
    receipt = {
        "schema": "t73_x_m1_framed_outer_interface_collars_v2_verification/v1",
...
def check_files(receipt):
    ...
    if (
        ...
        raise AssertionError("v2 outer collar verification binding changed")
...
def main():
    ...
    check_files(receipt)
    print(f"T73_X_M1_OUTER_COLLARS_V2={receipt['verdict']}")
