...
\begin{quote}\small
\textbf{审计更正。} 本节无条件验证的是端点模型标量 $2624$。把它写成
$D_3([v_T])$、把 Hattori 映射称为实际系数等价、以及把 $494$ 称为实际量子
次数的旧文字，均依赖当前未构造的 MWW 比较；完整端点匹配已经否定其所用的
literal split。实际比较及非零次数仍是条件假设。
\end{quote}

本节使第 4 节的有限层自洽。同一数据与 Johnson 替换领圈的几何绑定是
假设~\ref{hyp:P1}；整数 $2624$ 本身是当前冻结截断 Burau 计算的值，不是已识别的 MWW divided cubic。
...
$[\varepsilon^3]\ell(\rho(W)-I)u=-328$，从而
$D_3(v_T)=(-2)^3(-328)=2624$。非零结论不变。
一致使用 THXY 坐标得到 $-2496$；这是坐标校验，而非第二个几何类。值 $2624$ 属于
...
\begin{corollary}[对已识别图景的同伦球结论]
\label{cor:homotopy-sphere}
对由 P3/E13 与 $\Sigma_A^0$ 等同的 Johnson CS 柄图景 $X_J$，$\Sigma_A^0$ 是同伦
$4$-球。
\end{corollary}

\begin{proof}
由 \eqref{eq:determinants}，$A\in\mathrm{SL}(3,\Z)$ 且 $\det(A-I)=1$。
等同 $X_J\cong\Sigma_A^0$ 即所构造的 CS 柄图景；Iwaki 命题~2.1 \cite{Iwaki2024} 于是适用。
\end{proof}
...
对 Johnson 替换领圈，引理~\ref{lem:C1} 给出实际的系数等价 \eqref{eq:HattoriActual}。
...
\eqref{eq:ell-eps-series}，故 $D_3([v_T])=(-2)^3(-328)=2624$。
\end{proof}
...
\begin{proof}
引理~\ref{lem:HH0-descent} 将量子 shadow 检测器通过系数迹下降。定理~\ref{thm:Cdischarge} 与
引理~\ref{lem:quotient-detector} 将其通过 $q_{01}$ 下降。定理
\ref{thm:Sdischarge} 与同一商引理将其通过 $q_{12}$ 下降。
