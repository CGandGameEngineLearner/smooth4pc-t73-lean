...
\end{equation}
较早草稿混用两套 endpoint 坐标表并报告 $-59072$；在第~\ref{sec:exact-algebra} 节实际 endpoint 运输下，截断 Burau 值为 $2624$。该整数目前仅为这个几何绑定 Burau 计算的值；尚未与 MWW divided cubic 识别。\footnote{独立重放：\texttt{scripts/recompute\_t73\_delta3.py --check}。}
...
形式代数正确捕获最后一句。下文记录 P0、C、S 所需几何识别；每一项仍为 \Open。
...
第~\ref{sec:johnson-presentation}--\ref{sec:final-identifications} 节记录的几何输入仍为 \Open，且仍待组装为 Lean 中 \texttt{ExternalGeometry} 的单一居民；对 Johnson 候选，该组装为 \Open。
