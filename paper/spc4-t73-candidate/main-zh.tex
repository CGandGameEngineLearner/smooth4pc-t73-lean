...
整数 $2624$ 是当前冻结截断 Burau 计算在公开领圈词上的值，不是已识别的 MWW divided cubic。
...
\end{equation}
较早草稿混用两套 endpoint 坐标表并报告 $-59072$；在 \ref{sec:exact-algebra} 节领圈 endpoint 下冻结截断 Burau 值为 $2624$。该整数目前仅是冻结 Burau 计算的值，尚未与 MWW divided cubic 识别。\footnote{独立重放：\texttt{scripts/recompute\_t73\_delta3.py --check}。}

\begin{proposition}[计算的精确适用范围]\label{prop:scope}
方程~\eqref{eq:burau} 涉及 \eqref{eq:u} 与 \eqref{eq:ell} 的领圈 endpoint 下截断 Burau 模型。该整数是冻结 Burau 标量。将其与 MWW divided cubic 等同是定理~\ref{hyp:P1} 的内容，该定理仍为 \Open。
\end{proposition}
