...
\texttt{ExternalGeometry} 有实例时，定理~\ref{thm:A} 给出形式化光滑阻碍。下文数学证书完成假设~\ref{hyp:P0}--\ref{hyp:P3}；尚缺的是把这些解析对象装配成 Lean 居民。
\end{proof}
...
对 Johnson 替换，数学证书完成假设~\ref{hyp:P0}--\ref{hyp:P3} 并识别 $X_J\cong\Sigma_A^0$。Lean 开发仍只证明条件蕴含，因为解析性的 MWW 对象与映射尚未构造成 \texttt{ExternalGeometry} 居民；本文不声称完全由 Lean 验证的反例。独立重放命令列于附录~\ref{sec:reproducibility-extended}。
\appendix
