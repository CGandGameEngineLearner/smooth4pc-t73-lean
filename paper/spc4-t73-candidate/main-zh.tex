...
有限证书重构 Johnson 表示数据、公开 detector braid、端点模型中的整数
$2624$，以及所选系数外部的完整源端与目标端端点表。后两张表暴露八条
wrong-side connector，从而否定先前提出的相对四球 literal
two-representable split。相应的 $223$ 仅是该反事实目标的分次账本；历史值
$494$ 与 $223$ 目前都没有被证明是实际 MWW 类的次数。
Artin--Magnus 证书与 pure-braid Andreadakis 定理确立公开 braid 词的三阶性质。
若另行给出某个非零次数中的实际齐次 MWW 比较、全部 beta/psi 与三柄自然性
以及四柄绑定，则抽象论证会阻碍 $\Sigma_A^0$ 与 $S^4$ 微分同胚。
这些接口尚未构成 Lean 居民，故形式定理仍为条件式；本文不宣称得到光滑
四维 Poincaré 猜想的反例。
\end{abstract}
...
\begin{quote}\small
\textbf{版本说明（2026-09-05）。}
本中文稿的若干后续 C/S 推导尚未逐段同步英文主稿
\texttt{main.tex} 的最新审计更正；凡与“八条 wrong-side connector、实际
coend/currying map 与次数均为 OPEN”冲突的无条件叙述，均为历史文字而非当前
结论。当前可发布的逻辑状态以英文主稿及运行审计为准。
\end{quote}
\section{引言}
...
固定流形标签集合、有理向量空间分次族 $G(M,q)$、非零整数 $q_*$、标签 $X$ 与 $S^4$，以及谓词
...
\begin{hypothesis}[实际系数比较，C]\label{hyp:P1}
实际完备 MWW 系数迹中存在量子次数为 $q_C$ 的齐次类 $x_C$，并且在每个缆态
都有到 BPW/BHPW 端点模型的链层面比较族。该比较对两个系数作用、每个 beta/psi
映射以及远离 $B$ 的边界 cobordism 自然。在另行复合外置 cup
$E_{86}\to E_{88}$ 与端点 cap 后，divided cubic 通过所有关系下降，并在 $x_C$
上取值 $2624$。本假设包括尚缺的 $z$-coend gluing/currying map、任何重连胞腔
及其完整 Euler、framing 与分次账本。
\end{hypothesis}

完整端点构造器无条件给出的是 $1260$ 个端点、$630$ 条弧以及八条
wrong-side connector；它没有构造本假设中的比较，也没有确定 $q_C$。
...
在假设~\ref{hyp:P2} 的反向 $1$-柄 $3$-柄之后，剩余边界为 $S^3$。沿该
$S^3$ 附着 $4$-ball 得闭 $4$-流形 $X_J$。MWW 命题~3.4 给出四柄同构，
推论~3.5 给出 $\mathcal S^2_0(S^4)\cong\mathbb Z$ 集中在 bidegree
$(0,0)$，故 quantum-$q_C$ 分项消失。微分同胚诱导分次保持等价。
$\det A=\det(A-I)=1$ 只给出标准 Cappell--Shaneson 构造的同伦球面判据；
识别 $X_J\cong\Sigma_A^0$ 来自 P0 构造。
\end{hypothesis}

\begin{theorem}[条件式 trace-73 阻碍]\label{thm:joined}
假设 P0、C、S 与 P3 的全部数学输入成立，包括实际 MWW 比较及
$X_J\cong\Sigma_A^0$ 的识别。则
Cappell--Shaneson 流形 $\Sigma_A^0$ 是光滑同伦 $4$-球面，且
\[
\Sigma_A^0\not\cong_{\mathrm{diff}}S^4.
\]
因此这些假设若成立，将给出光滑四维 Poincaré 猜想的反例。
\end{theorem}

\begin{proof}[定理~\ref{thm:joined} 的证明]
所假设的数据提供定理~\ref{thm:A} 的数学输入。故非零次数-$q_C$ 类阻碍与
$S^4$ 的微分同胚。Iwaki 命题~2.1 与 $\det(A-I)=1$ 独立表明
$\Sigma_A^0$ 是同伦球面。Lean 开发只形式化有限代数与抽象条件蕴含。
\end{proof}
...
\end{equation}
较早草稿混用两套 endpoint 坐标表并报告 $-59072$；在第~\ref{sec:exact-algebra} 节实际 endpoint 运输下，截断 Burau 值为 $2624$。把该值与实际 MWW divided cubic 行识别正是假设~\ref{hyp:P1} 的内容。\footnote{独立重放：\texttt{scripts/recompute\_t73\_delta3.py --check}。}

\begin{proposition}[计算的精确适用范围]\label{prop:scope}
方程~\eqref{eq:burau} 涉及 \eqref{eq:u} 与 \eqref{eq:ell} 的领圈 endpoint 下截断 Burau 模型。该整数为端点模型值；将其与显式 Johnson 表示的实际 MWW 行识别仍是假设~\ref{hyp:P1}。
\end{proposition}

\begin{proof}
Burau 计算给出整数；假设~\ref{hyp:P1} 才将其与 Johnson 替换领圈上的 MWW detector 等同。
\end{proof}
...
\begin{quote}\small
\textbf{审计更正。}
当前完整构造器在源端与目标端都保存了 $1260$ 个端点和 $630$ 条带框弧；
每侧为 $88$ 条 $Y$--$Z$ 弧与 $227$ 条 $Z$--$Z$ 弧。但是源端匹配中有
$4$ 条 $Y_-$--$Z_-$ 与 $4$ 条 $Y_+$--$Z_+$ 弧，故不存在逐点固定四个
插入球的 literal two-representable split 同痕。下述 Hattori 等价及
$494$ 分次推导是已被该端点表否定的历史路线，不是实际系数比较的证明。
真正需要的 $z$-coend/currying 映射、可能的重连胞腔、Blanchet 符号、Euler
次数与绝对量子次数仍为 OPEN；外置的 $P_{86}\to P_{88}$ cup 也不能补出这些
数据。
\end{quote}
...
对 Johnson 替换，正文给出 P0/E13 的纸面构造与上柄运输；端点构造器同时
否定了旧的 literal split。实际 C 比较、其次数 $q_C$ 与 S 的全态自然性仍为
假设，且未构造成 \texttt{ExternalGeometry} 的居民。因此当前只有条件式阻碍，
没有已完成的反例定理。独立重放命令列于附录~\ref{sec:reproducibility-extended}。
\appendix
...
P0/E13 的拓扑输入在正文中给出；C 与 S 的实际比较仍是数学假设，而不只是
Lean 包装尚缺。它们也尚未组装成 Lean 中 \texttt{ExternalGeometry} 的居民。
