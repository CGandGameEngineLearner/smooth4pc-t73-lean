...
因此 $\Sigma_A^0$ 是不微分同胚于 $S^4$ 的光滑同伦 $4$-球面，给出光滑
四维 Poincaré 猜想的数学反例。
...
主要贡献如下。
\begin{enumerate}
\item 构造保分裂 Johnson 提升、实际带 framing 的 AR link、两次 Kirby 消去 movie，以及从几何导出并等于公开 11340 字母词的六扫掠 44 通道词。
\item 为 C 构造全部 44 条乘积矩形与 227 个剩余圆；为 S 构造三张 dual-disk 迹曲面，其几何 core 数为 $(12578,1824,409)$。
...
定理~\ref{thm:P0discharge}、\ref{thm:Cdischarge}、\ref{thm:Sdischarge}，
连同假设~\ref{hyp:P3} 与 E13 识别，提供定理~\ref{thm:A} 的数学数据，故非零
次数-$494$ 类阻碍与 $S^4$ 的微分同胚。Iwaki 命题~2.1 及
$\det(A-I)=1$ 独立证明 $\Sigma_A^0$ 为同伦球面。Lean 只形式化有限代数与
抽象蕴含，尚未形式化该解析实例；这不使数学陈述成为条件式。
\end{proof}
...
\end{equation}
较早草稿混用两套 endpoint 坐标表并报告 $-59072$；在 \ref{sec:exact-algebra} 节的实际 endpoint 运输下，截断 Burau 值为 $2624$。第~\ref{sec:c-comparison} 节把该几何绑定值与显式 Johnson 表示的 MWW divided cubic 行识别。\footnote{独立重放：\texttt{scripts/recompute\_t73\_delta3.py --check}。}

\begin{proposition}[计算的精确适用范围]\label{prop:scope}
方程~\eqref{eq:burau} 涉及 \eqref{eq:u} 与 \eqref{eq:ell} 的领圈 endpoint 下截断 Burau 模型。该整数是几何绑定 Burau 标量；第~\ref{sec:c-comparison} 节完成其与显式 Johnson 表示的 MWW 行之识别。
\end{proposition}

\begin{proof}
Burau 计算给出整数，第~\ref{sec:c-comparison} 节的实际 44+227 乘积分解与 cocone 使其成为 Johnson 替换领圈上的 MWW detector。
\end{proof}
...
所需 Johnson restore 映射及分裂的集合意义保持在此中间阶段仍为 \Open，后续 assembly 将其关闭。
...
Johnson 侧位交换衣领及其固定边界 PL 运输是当时的下一中间门。
...
saddle 展开成具正 Jacobian 的显式环境 PL 胞腔在此阶段仍为 \Open。
...
通过。source/target 盘之间的 fiber transport 在此阶段仍为 \Open，故当时尚不声称
...
接合成在支撑外边界恒等的映射；paired-saddle 环境映射在此阶段仍为 \Open。
...
Johnson 侧位脊运输及重建 AR 附着链在此阶段仍为 \Open；旧 AR-link SHA 被拒绝。
...
显式序列把它绑定到 Restore 是当时的下一门，已由后续 lane-spine binding 完成。
...
假设~\ref{hyp:P0} 对显式 Johnson 表示成立。
\end{theorem}

\begin{proof}
保持 Heegaard 的 93 因子映射、七分量带 framing AR link、6 条与 1513 条 Kirby band、实际 44 通道切割、端点返回图及末端词比较给出所需桥梁、消去与 cabling 数据；E13 链识别闭图与 $\Sigma_A^0$。
\end{proof}
...
假设~\ref{hyp:P1} 对显式 Johnson 表示成立。
\end{theorem}

\begin{proof}
44 条实际 y/z 矩形与 227 个实际剩余圆运输给出引理~\ref{lem:C1} 的系数双模识别；引理~\ref{lem:C2} 与 Reynolds cocone 验证两侧作用及每个有限 cable 状态。
\end{proof}
...
\]
不消去非相邻反向副本而追踪有向 dual-disk 词，得到实际几何计数
\[
(b_1,b_2,b_3)=(12578,1824,409).
\]
其乘积法向副本 profile 与对应的
$\epsilon^{\otimes b_j}\Delta^{b_j-1}$ 计算已经显式给出。相对 PL 边界映射
以层级形式表示：先应用全部 93 个环境 dual-disk 因子，再应用两次 Kirby
边界映射的 6 条与 1513 条 band，并把所得每个边界副本绑定到实际 owner。
该矩阵本身已不再是外部输入：把 dual handlebody 的三张方形压缩盘逐因子
运输过全部 93 个 Johnson 环境映射，得到
$\wedge^2A=A^{-T}$，mapping-torus 边界因而为 $I-A^{-T}$。独立 surface
transport verifier 重放两次 Kirby 衣领，并拒绝缺少任一 band 的变异。
...
完整混合 braid 映射形如 \(P(I+O(h))\)。置换 \(P\) 刚被抵消，以 \(I+O(h)\) 预复合或后复合始于 \(h^3\) 阶的行仅改变到 \(h^4\) 阶及以上。故常数计算 (32) 是除法三次行所见的候选计算。对三个实际几何值 $b_j=12578,1824,409$，有限 Frobenius movie 分别含 $b_j-1$ 个 coproduct saddle 与 $b_j$ 个 counit，并重现 (32)。
\end{proof}

端点交换方块是引理~\ref{lem:Sendpoint} 中上述迭代 coproduct--counit 模型，并已绑定实际层级 $W_2$ 曲面。
\[
\begin{array}{ccc}
...
假设~\ref{hyp:P2} 对显式 Johnson 表示成立。
\end{theorem}

\begin{proof}
三张 H1 压缩盘被运输过全部 93 个 Johnson 因子及 1519 条 Kirby band。其几何边界词长度为 $12578,1824,409$；所得穿孔球面 movie 在实际 $\partial W_2$ 上实现引理~\ref{lem:Sendpoint} 的映射。
\end{proof}
\section{最终识别}\label{sec:final-identifications}

MWW 定理~3.10 将迭代商与三柄后模等同，命题~3.4 给出四柄同构 \cite{ManolescuWalkerWedrich2023}。定理~\ref{thm:P0discharge}、定理~\ref{thm:Cdischarge} 与定理~\ref{thm:Sdischarge} 完成假设~\ref{hyp:P0}--\ref{hyp:P2}。对 Johnson 替换四柄图景 $X_J$，沿每个 belt 球面附着三柄恢复边界 $S^3$，附着 PL $4$-ball；实际 E13 柄链识别 $X_J$ 与 $\Sigma_A^0$。

\begin{remark}[形式化边界]\label{rem:lean-boundary}
Lean 开发 \cite{Lean4} 将定理~\ref{thm:A} 证明为从结构 \texttt{ExternalGeometry} 与 \texttt{CSExternalGeometry} 出发的条件蕴含。它检验矩阵算术、几何绑定 Burau 值 $2624$、次数 $494$ 与抽象商引理，但不构造这些解析几何居民。附录~\ref{app:lean-fields} 列出与 Lean 字段的对应。
\end{remark}
...
对 Johnson 替换，数学证书完成假设~\ref{hyp:P0}--\ref{hyp:P3} 并识别 $X_J\cong\Sigma_A^0$，从而给出论文层面的数学反例定理。Lean 开发仍只证明条件蕴含，因为解析性的 MWW 对象与映射尚未构造成 \texttt{ExternalGeometry} 居民；因此该结论有待专家审阅，尚不是完全由 Lean 验证的定理。独立重放命令列于附录~\ref{sec:reproducibility-extended}。
\appendix
