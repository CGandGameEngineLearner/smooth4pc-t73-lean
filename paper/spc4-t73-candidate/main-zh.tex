...
$\wedge^2A=A^{-T}$，mapping-torus 边界因而为 $I-A^{-T}$。尚未完成的是
把这些盘迹曲面运输过两次 Kirby 消去衣领。
...
完整混合 braid 映射形如 \(P(I+O(h))\)。置换 \(P\) 刚被抵消，以 \(I+O(h)\) 预复合或后复合始于 \(h^3\) 阶的行仅改变到 \(h^4\) 阶及以上。故常数计算 (32) 是除法三次行所见的候选计算。对三个非零值 $b_j=9920,1430,311$，有限 Frobenius movie 分别含 $b_j-1$ 个 coproduct saddle 与 $b_j$ 个 counit，并重现 (32)。但穿孔曲面尚未经缺失的相对 $\partial W_2$ 识别运输，故这还不是实际 MWW 映射。
\end{proof}
