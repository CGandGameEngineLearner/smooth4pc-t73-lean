...
衣领仍为 \Open。
对最终衣领，用最短 dual-face 路径连接残余 add/remove 球，再加入一层
完整 agreement 衣领。四个所得支撑分别含 $129,125,180,179$ 个四面体；
每个边界为球面并有显式到点坍缩。current/target membership 在外边界上
一致，两片内部曲面具有完全相同的边界边集。负 move 的每片曲面是圆盘
与环带的不交并，正 move 则是两个圆盘与环带的不交并；所有分量均为
平面曲面，且支撑包围盒到每个保护球中心的距离至少为 $1$。但这尚未给出
相对支撑同胚：边界边的并集虽相同，这些边在圆盘与环带组件间的分组在
source/target 两侧不同；固定边界的同胚不能改变该分组。因此 paired
saddle 支撑及其环境 PL 胞腔映射仍为 \Open。
不过该差异是有限且标准的：负模板的公共边界含三条圆，单个换位把
source 组件分组送到 target 分组；正模板有四条圆，最小置换是两个不交
换位的乘积。因此所需内边界映射是一个或两个球面半转；其实际放置及
外层固定边界极坐标衣领仍待构造。
