...
衣领仍为 \Open。
对最终衣领，用最短 dual-face 路径连接残余球并加入一层 agreement，得到
大小为 $129,125,180,179$ 的候选子复形。它们可坍缩到点，且到保护球
中心的距离至少为 $1$。第一、第二与第四个具有流形球面边界；含 $180$
个四面体的正 prefix-first 候选有一条重数为四的边，必须拒绝。四者仍都
不充分：即使对三个三球支撑，公共边界圆在圆盘/环带组件间的分组也在
source/target 两侧不同，尚无固定边界同胚。
不过该差异是有限且标准的：负模板的公共边界含三条圆，单个换位把
source 组件分组送到 target 分组；正模板有四条圆，最小置换是两个不交
换位的乘积。因此所需内边界映射是一个或两个球面半转；其实际放置及
外层固定边界极坐标衣领仍待构造。所需内边界映射是一个或两个球面半转；
对三个流形候选，圈置换还保持球面嵌套树。正 prefix-first 仍需细分正则
邻域；所有情形仍需实际放置圈半转及固定边界极坐标衣领，故保持 \Open。
