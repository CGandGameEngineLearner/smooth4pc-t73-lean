...
not an identification $X_J\cong\Sigma_A^0$.
\end{theorem}
...
MWW citations.  The picture is not a triangulation of a four-dimensional
$W_3$, and $X_J$ is not identified with $\Sigma_A^0$.
\end{proof}

\begin{theorem}[Conditional trace-73 theorem]\label{thm:joined}
If the remaining identification of the Johnson replacement closed
$4$-manifold $X_J$ with $\Sigma_A^0$ were supplied as an object meeting the
repository acceptance tests, then the trace-73 Cappell--Shaneson sphere
would satisfy
\[
\Sigma_A^0\simeq S^4
\quad\text{and}\quad
...
\]
P0, C, S and the MWW four-handle layer are discharged for the Johnson
replacement; $X_J\cong\Sigma_A^0$ is not.  The theorem is therefore
...
$X_J\cong\Sigma_A^0$ remains Open, so the candidate is not instantiated as
the standard Cappell--Shaneson sphere.
\end{proof}
...
\]
The identification $X_J\cong\Sigma_A^0$ remains Open, so the iterated
quotient is not identified with $\mathcal S^2_0(\Sigma_A^0)$.
$\det(A-I)=1$ remains the finite homotopy-sphere criterion for the
standard Cappell--Shaneson construction, not a C/S closure of $X_J$.
...
P3/E12 & \Discharged & MWW Corollary~3.5 as a statement about $S^4$; quantum degree $494$ vanishes. \\
P3/E13 & \Partial & $\det(A-I)=1$ is proved; $X_J\cong\Sigma_A^0$ remains Open. \\
\bottomrule
\end{tabularx}
\end{center}
...
so Theorem~\ref{thm:A} and Lean's
...
\texttt{s4DegreeZero} & E12 as MWW Corollary~3.5 about $S^4$; \texttt{S4ReductionData} uninhabited. \\
\texttt{diffeomorphismEquiv} & Graded diffeomorphism invariance of lasagna modules. \\
\texttt{matrixConditionsToHomotopySphere} & E13 determinants; identification with $\Sigma_A^0$ remains Open. \\
\bottomrule
\end{tabularx}
\end{center}

These remain Open or Partial as in the status table: $X_J\cong\Sigma_A^0$ is
Open.  The Lean theorem consumes no stronger statement, and it records the
