...
\begin{enumerate}
\item We construct a splitting-preserving Johnson lift, its actual framed AR
link, both Kirby cancellation movies, and an exact geometry-derived six-sweep
44-channel word equal to the public 11340-letter word.
\item We construct all 44 product rectangles and 227 leftover circles for C,
and the three dual-disk-track surfaces for S with geometric core counts
$(12578,1824,409)$.
...
The candidate-specific inputs are stated and discharged below.  Their packaging
as inhabitants of the Lean interfaces is not part of the formal development.
...
Theorem~\ref{thm:P0discharge} assembles these data.
\end{proof}
...
Section~\ref{sec:c-comparison} constructs the 44 actual product rectangles,
227 source-bound leftover circles, endpoint transport and complete beta/psi
cocone.  Theorem~\ref{thm:Cdischarge} identifies the resulting row with the
MWW divided cubic.
\end{proof}
...
Section~\ref{sec:s-closure} transports the three H1 compression disks through
all 93 Johnson factors and 1519 Kirby bands, constructs the actual punctured
hemisphere movies, and verifies the complete MWW coequalizer.
Theorem~\ref{thm:Sdischarge} records the result.
\end{proof}
...
Theorems~\ref{thm:P0discharge}, \ref{thm:Cdischarge}, and
\ref{thm:Sdischarge}, together with Hypothesis~\ref{hyp:P3} and the E13
identification, supply the mathematical data of Theorem~\ref{thm:A}.  Hence
...
finite algebra and the abstract implication but not this analytic
instantiation; that formalization boundary does not make the mathematical
...
$-59072$; with the actual endpoint transport of Section~\ref{sec:exact-algebra}
the truncated Burau value is $2624$.  Section~\ref{sec:c-comparison}
identifies this geometry-bound value with the MWW divided cubic row.\footnote{Independent replay:
...
Section~\ref{sec:c-comparison} identifies its divided cubic with the MWW row
for the explicit Johnson presentation.
\end{proposition}

\begin{proof}
The Burau calculation supplies an integer.  Theorem~\ref{hyp:P1} and
Section~\ref{sec:c-comparison} identify it with the MWW detector on the
Johnson replacement collar.
\end{proof}
...
give the Johnson-generator replacement that proves Hypothesis~\ref{hyp:P0}.
...
\]
are nonzero, but they are not the selected cubic.  The geometry-bound public receipt and
Lean record the truncated Burau scalar
\begin{equation}\label{eq:selectedD3}
D_3([v_T])=2624
\end{equation}
after the endpoint-indexing erratum.  The complete two-handle cocone below
identifies this value with the selected MWW class, and the three-handle
surface movies prove that it descends through S.
...
Hypothesis~\ref{hyp:P1} holds for the explicit Johnson presentation.
\end{theorem}

\begin{proof}
The 44 actual y/z rectangles and 227 actual leftover-circle transports give
the coefficient-bimodule identification in Lemma~\ref{lem:C1};
Lemma~\ref{lem:C2} and the Reynolds cocone verify both actions and every
finite cable state.
\end{proof}
...
Hypothesis~\ref{hyp:P2} holds for the explicit Johnson presentation.
\end{theorem}

\begin{proof}
The three H1 compression disks are transported through all 93 Johnson factors
and all 1519 Kirby bands.  Their geometric boundary words have lengths
$12578,1824,409$; the resulting punctured-sphere movies realize the maps of
Lemma~\ref{lem:Sendpoint} on the actual $\partial W_2$.
\end{proof}
...
Theorem~\ref{thm:Cdischarge}, and Theorem~\ref{thm:Sdischarge} discharge
Hypotheses~\ref{hyp:P0}--\ref{hyp:P2}.  For the Johnson
...
Corollary~3.5.  The actual E13 handle chain identifies $X_J$ with $\Sigma_A^0$.
...
\texttt{CSExternalGeometry}.  It checks the matrix arithmetic, the geometry-bound Burau value
...
are discharged in the paper.  They have not been assembled into a single
Lean inhabitant of \texttt{ExternalGeometry}; that is a formalization gap,
not a hypothesis in the mathematical theorem.
