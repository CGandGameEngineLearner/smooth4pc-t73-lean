...
from those external structures.
\end{abstract}
...
isotopy and the comparison maps.  Hypothesis~\ref{hyp:P2} is discharged for the Johnson replacement.
\end{proof}
...
exactly and has relative $r_{yz}$ coefficient zero as a finite P0d fact.
After P0c, $\operatorname{lk}(m_2,r_{yz})$ must still be read from a reduced
PD or normal-field movie; that object is absent.
...
No historical-PD identity is asserted.
\end{proof}
...
Hypothesis~\ref{hyp:P2} is discharged for the Johnson replacement.
...
Lemmas~\ref{lem:C1} and~\ref{lem:C2}.  Hypothesis~\ref{hyp:P2} is discharged for the Johnson replacement.
\end{proof}
...
P0d & \Partial & 93-bit Johnson lift matches the public 11340-letter word; finite fact only. \\
...
C3 & \Open & Not used to infer S. \\
P2/E7 & \Open & Closed HJ Theorem~5.3 is not used as ``$B$ is fixed''. \\
...
These remain Open or Partial as in the status table: Lean
