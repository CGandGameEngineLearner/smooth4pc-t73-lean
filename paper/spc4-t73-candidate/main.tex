...
recorded sphere columns instead give the candidate core-disk counts
\[
(b_1,b_2,b_3)=(9920,1430,311).
\]
Their product-normal copy profiles and the corresponding
$\epsilon^{\otimes b_j}\Delta^{b_j-1}$ calculations are explicit, but the
relative PL boundary map carrying those copies from the actual five-component
attaching link to the reversed model remains \Open.
...
mapping-torus boundary is $I-A^{-T}$.  What remains open is the transport of
these disk-track surfaces through the two Kirby cancellation collars.
...
$b_j=9920,1430,311$, the finite Frobenius movies have respectively
$b_j-1$ coproduct saddles and $b_j$ counits and reproduce (32).  This is not
yet an actual MWW map because the punctured surfaces have not been transported
through the missing relative $\partial W_2$ identification.
\end{proof}

The endpoint exchange square is the above iterated coproduct--counit model of
Lemma~\ref{lem:Sendpoint}; its actual-$W_2$ binding remains \Open.
\[
\begin{array}{ccc}
