...
cancellations of the reversed $1$-handles, a PL $4$-ball $I^4$ of Euler
characteristic $1$ with boundary $S^3$, the finite determinants, and the
MWW citations.  \texttt{scripts/certify\_t73\_e12\_s4.py} computes the
empty-link Khovanov complex over $\mathbb Q$ and the standard $S^4$ obtained
by gluing two copies of that $I^4$ along $S^3$.  The picture is not a
triangulation of a four-dimensional
$W_3$.  Identification $X_J\cong\Sigma_A^0$ is
\texttt{scripts/certify\_t73\_e13\_close.py}, not this four-handle certificate.
\end{proof}
...
PL $4$-ball is attached along that $S^3$, and Corollary~3.5 kills the
$S^4$ summand in quantum degree $494$.  The certificate is
...
P3/E12 & \Discharged & Empty-link Khovanov over $\mathbb Q$, PL $S^4=I^4\cup_{S^3}I^4$ of Euler characteristic $2$, MWW Proposition~3.4; quantum degree $494$ vanishes.  Lean inhabits \texttt{S4ReductionData} on the empty-link control universe; the candidate \texttt{ExternalGeometry} remains uninhabited. \\
...
\texttt{s4DegreeZero} & E12 empty-link control inhabits \texttt{S4ReductionData} on \texttt{EmptyKhQ}; candidate transport remains a parameter. \\
