...
Hypothesis~\ref{hyp:P1}; the integer $2624$ itself is independent of that
binding.
...
updated together.  The value $2624$ belongs to the currently frozen data.
If the Johnson collar word changes, the numerical value must be recomputed.
The superseded public PDF had SHA-256 prefix \texttt{8E621FD9}.
...
\end{pmatrix}.
\end{equation}
These are checked in \path{Smooth4PC/T73CSAlgebra.lean} by ring normalization.
The columns in \eqref{eq:spherecols} are the negated class columns of $C$;
$\det C=1$ is \path{T73.sphereDet_eq_one}.
...
homotopy equivalence with $S^4$.  Those implications are represented by
separate fields in \path{Smooth4PC/T73CSTopology.lean}; see
Section~\ref{sec:published-results}.  This paper uses Iwaki
...
pair.  This is the quotient implemented in
\path{Smooth4PC/CoefficientTrace.lean}.
\end{definition}
...
For the Johnson replacement collar, Lemma~\ref{lem:C1} supplies the actual
coefficient equivalence \eqref{eq:HattoriActual}.  The selected class and
...
$|B_{44}|=11340$ and $|B_{88}|=45360$.  The canonical JSON SHA-256 values
are
\[
...
and
\[
\hash{C13D0A3BA9B05F41A6D2C5B4AB12DDEC}
     {DABEFC1883835891AD3BD2B8955B5FFB}.
\]
The Johnson reconstruction verifier recovers the same $11340$-letter word
letter-for-letter.
\end{proposition}
...
u=e_2-e_{87}.
\end{equation}
Both formulas use the collar-coordinate table
\path{data/B88_POSITION_TO_PASSAGE_TABLE.json}; its SHA-256 is
\[
\hash{119C7E9E74AE6C820DA72A84CDFD5D44}
     {5D81E6C3AACCC209C25D37C323961508}.
\]
...
\begin{proof}
The deterministic algorithm in \texttt{scripts/recompute\_t73\_delta3.py}
traverses $B_{88}$ from right to left, reduces modulo $\eps^7$, and
contracts with \eqref{eq:ell}.  The input SHA-256 is
\[
\hash{0F0F4A568D966367313C29C25CEDDE4}
     {FD9ABAD32B9375519E9CDF9168AE0A764}.
\]
Lean stores \texttt{cubicBase}$=-328$ and proves
\texttt{computedCubic\_eq\_2624}.
\end{proof}
...
\end{align}
Adding gives $-44+227+315-4=494$.  Lean proves \texttt{computedDegree}.
Appendix~\ref{app:grading} collects the same table without abbreviation.
\end{proof}
...
coefficient trace.  Theorem~\ref{thm:Cdischarge} and
Lemma~\ref{lem:quotient-detector} descend it through $q_{01}$.  Theorem
\ref{thm:Sdischarge} and the same quotient lemma descend it through $q_{12}$.
